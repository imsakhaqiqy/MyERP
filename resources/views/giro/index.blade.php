...
@section('breadcrumb')
  <ol class="breadcrumb">
    ...
    <li><a href="{{ URL::to('sales-giro') }}"><i class="fa fa-dashboard"></i> List Giro</a></li>
...
@section('content')
  ...
  <div class="modal fade" id="modal-approve-sales-giro" tabindex="-1" role="dialog" aria-labelledby="modal-approve-sales-giroLabel">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
      {!! Form::open(['url'=>'approveSalesGiro', 'method'=>'post']) !!}
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
          <h4 class="modal-title" id="modal-approve-sales-giroLabel">Confirmation</h4>
        </div>
        <div class="modal-body">
          ...
          <input type="hidden" id="sales_giro_id" name="sales_giro_id">
          <input type="hidden" id="sales_giro_no_giro" name="sales_giro_no_giro">
        </div>
...
@section('additional_scripts')
  <script type="text/javascript">
    var tableGiro =  $('#table-list-giro').DataTable({
      processing :true,
      serverSide : true,
      ajax : '{!! route('datatables.getSalesGiros') !!}',
      columns :[
    ...
    tableGiro.on('click', '.btn-approve-giro', function (e) {
      ...
      $('#sales_giro_id').val(id);
      $('#payment-invoice-giro-to-approve').text(no_giro);
      $('#sales_giro_no_giro').val(no_giro);
      if(status == 'pos'){
            $('#modal-approve-sales-giro').modal('show');
      }
      return false;
    });
